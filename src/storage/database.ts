  ...
  constructor() {
    ...
    this.version(15)
      ...
      .upgrade(async (transaction) => {
        ...
        await transaction.table("characterCreationDrafts").toCollection().modify((draft) => {
          draft.character ??= {};
          draft.character.portraitDataUrl ??= "";
        });
      });
    ...
    this.version(16)
      ...
      .upgrade(async (transaction) => {
        await transaction.table("characterSheets").toCollection().modify((sheet) => {
          sheet.spellSlotRecovery ??= {};
          sheet.pactMagicSlots ??= {};
          sheet.pactMagicSlotsUsed ??= {};
          sheet.pactMagicRecovery ??= {};
        });
      });
    ...
    this.version(20)
      .stores({
        characters: "id, name, updatedAt, createdAt, archivedAt, favorite, lastOpenedAt",
        characterSheets: "characterId, updatedAt",
        inventoryContainers: "id, characterId, [characterId+sortOrder], updatedAt",
        inventoryItems: "id, characterId, containerId, [characterId+containerId], updatedAt",
        spellbooks: "characterId, updatedAt",
    ...
    this.version(21)
      ...
      .upgrade(async (transaction) => {
        await transaction.table("spells").toCollection().modify((spell) => {
          ...
          spell.imported ??= false;
          spell.importSourceName ??= "";
          spell.importSourceIndex ??= null;
          spell.importVariantKey ??= "";
          spell.levelKnown ??= true;
        });
      });
    ...
    this.version(22)
      ...
      .upgrade(async (transaction) => {
        const centered = { zoom: 1, offsetX: 0, offsetY: 0, version: 1, updatedAt: null };
        await transaction.table("characters").toCollection().modify((character) => {
          character.portraitImageId ??= character.portraitDataUrl ? `legacy:${character.id}` : "";
          character.portraitTransform ??= { ...centered };
        });
        await transaction.table("characterCreationDrafts").toCollection().modify((draft) => {
          draft.character ??= {};
          draft.character.portraitImageId ??= draft.character.portraitDataUrl ? "legacy:creation-draft" : "";
          draft.character.portraitTransform ??= { ...centered };
        });
      });
    ...
    this.version(23)
      ...
      .upgrade(async (transaction) => {
        await transaction.table("characters").toCollection().modify((character) => {
          character.portraitTransform ??= {};
          character.portraitTransform.mode ??= "cover";
          character.portraitTransform.naturalWidth ??= null;
          character.portraitTransform.naturalHeight ??= null;
        });
        await transaction.table("characterCreationDrafts").toCollection().modify((draft) => {
          draft.character ??= {};
          draft.character.portraitTransform ??= {};
          draft.character.portraitTransform.mode ??= "cover";
          draft.character.portraitTransform.naturalWidth ??= null;
          draft.character.portraitTransform.naturalHeight ??= null;
        });
      });
    ...
    this.version(26).stores({
      ...
      pdfBookmarks: "id, documentId, [documentId+page], createdAt",
      settings: "id",
      syncOutbox: "id, characterId, status, timestamp",
      syncTombstones: "id, characterId, [characterId+tableName], deletedAt",
      syncMetadata: "id",
    });
