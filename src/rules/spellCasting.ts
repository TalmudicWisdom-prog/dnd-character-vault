...
import { changeUsedSpellSlots, remainingSpellSlots } from "./spellSlots";
...
export function canCastSpellWithSlot(sheet: CharacterSheet, spell: Spell, slotLevel: number | null) {
  if (spell.level === 0) return true;
  if (!slotLevel || slotLevel < spell.level || slotLevel > 9) return false;
  const maximum = sheet.spellSlots[String(slotLevel)] ?? 0;
  const used = sheet.spellSlotsUsed[String(slotLevel)] ?? 0;
  return remainingSpellSlots(maximum, used) > 0;
}

export function consumeSpellSlot(sheet: CharacterSheet, spell: Spell, slotLevel: number | null) {
  if (spell.level === 0) return sheet;
  if (!canCastSpellWithSlot(sheet, spell, slotLevel)) throw new Error("No valid spell slot remains for that casting level.");
