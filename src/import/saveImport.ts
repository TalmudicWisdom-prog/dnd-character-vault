import type { CharacterImportDraft, ImportSaveMode, ImportSessionFile } from "../domain/import";
import { characterSheetSchema, inventoryItemSchema, pdfDocumentSchema, type CharacterDraft, type CharacterSheet } from "../domain/models";
...
import { persistImportedSpells, type PersistImportedSpellsResult } from "./spellPersistence";
...
function mergeNames(existing: string, names: string[]) {
...
async function linkSourcePdfs(characterId: string, files: ImportSessionFile[]) {
  let linked = 0;
  for (const file of files) {
    if (file.type !== "application/pdf" && !file.name.toLocaleLowerCase().endsWith(".pdf")) continue;
    const duplicate = await db.pdfDocuments.filter((document) =>
      document.fileName === file.name && document.size === file.size && document.characterIds.includes(characterId)).first();
    ...
    const document = pdfDocumentSchema.parse({
      ...
      fileName: file.name,
      size: file.size,
      gameSystem: "D&D 5E character import",
    ...
    await db.pdfFiles.add({ documentId: document.id, data: file.data });
    linked += 1;
  }
  return linked;
}

export type SaveCharacterImportResult = {
  characterId: string;
  spells: PersistImportedSpellsResult;
  linkedPdfCount: number;
};

export async function saveCharacterImport(
  draft: CharacterImportDraft,
  mode: ImportSaveMode,
  existingCharacterId?: string,
  sourceFiles: ImportSessionFile[] = [],
): Promise<SaveCharacterImportResult> {
  return db.transaction("rw", [
    db.characters, db.characterSheets, db.inventoryContainers, db.inventoryItems,
    db.spellbooks, db.spells, db.pdfDocuments, db.pdfFiles,
  ], async () => {
    ...
    if (draft.spellcastingAbility?.include) sheet.spellcastingAbility = draft.spellcastingAbility.value;
    if (draft.spellSaveDc?.include) sheet.spellSaveDc = Math.max(0, Math.round(draft.spellSaveDc.value));
    if (draft.spellAttackBonus?.include) sheet.spellAttackBonus = Math.round(draft.spellAttackBonus.value);
    ...
    if (draft.spellsAndNotes.include) sheet.spellNotes = appendNotes(sheet.spellNotes, "Imported PDF notes", draft.spellsAndNotes.value);
    const importedRecords = draft.importedSpells?.include ? draft.importedSpells.value : [];
    sheet.cantrips = mergeNames(sheet.cantrips, importedRecords.filter((spell) => spell.level === 0).map((spell) => spell.name));
    sheet.preparedSpells = mergeNames(sheet.preparedSpells, importedRecords.filter((spell) => spell.level !== 0 && (spell.prepared || spell.alwaysPrepared)).map((spell) => spell.name));
    sheet.notes = sheet.notes.slice(0, 50000);
    sheet.spellNotes = sheet.spellNotes.slice(0, 30000);
    ...
    const spells = importedRecords.length
      ? await persistImportedSpells(characterId, importedRecords, draft.sourceName, draft.spellcastingClass?.value ?? draft.characterClass.value)
      : { detected: 0, imported: 0, matched: 0, custom: 0, skippedExisting: 0 };
    const linkedPdfCount = await linkSourcePdfs(characterId, sourceFiles);
    return { characterId, spells, linkedPdfCount };
  });
}
