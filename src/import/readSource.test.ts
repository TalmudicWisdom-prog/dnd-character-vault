  ...
  return {
    ...
    getFieldObjects: options.fieldFailure
      ...
      : async () => ({
        ...
        spellCastingAbility0: [{ value: "WIS" }], spellSaveDC0: [{ value: "16" }], spellAtkBonus0: [{ value: "+8" }], spellCastingClass0: [{ value: "Druid" }],
        spellHeader0: [{ value: "=== CANTRIPS ===" }], spellName0: [{ value: "Druidcraft" }], spellPrepared0: [{ value: "P" }],
        spellHeader1: [{ value: "=== 1st LEVEL ===" }], spellName2: [{ value: "Goodberry" }], spellPrepared2: [{ value: "O" }],
        spellName223: undefined,
      }),
...
describe("fillable multi-page PDF character extraction", () => {
  it("extracts an eight-page AcroForm sheet and normalizes the character preview", async () => {
    ...
    const draft = extractCharacterText(parsed.rawText, "Akiva Character D&D.pdf", parsed.spellData);

    expect(parsed.pageCount).toBe(8);
    expect(parsed.formFieldCount).toBeGreaterThan(20);
    expect(parsed.spellData.spells).toHaveLength(2);
    expect(parsed.spellData.spellcasting).toEqual({ ability: "wis", saveDc: 16, attackBonus: 8, sourceClass: "Druid" });
